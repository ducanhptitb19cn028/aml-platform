...
/**
 * The outbox row. Persisted in the same transaction as the aggregate
 * that produced the event, then dispatched to Kafka by a separate
 * worker.
 *
 * @JdbcTypeCode(SqlTypes.JSON) maps the payload to Postgres JSONB —
 * gives us indexable, queryable event data without giving up the
 * "events are opaque to the messaging layer" property.
 */
@Entity
@Table(name = "outbox")

...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  stage/.style={draw,rectangle,rounded corners=3pt,minimum height=1.0cm,
                minimum width=2.0cm,align=center,font=\scriptsize,fill=orange!8,
                draw=orange!50},
  input/.style={draw,rectangle,rounded corners=3pt,minimum height=1.0cm,
                minimum width=1.4cm,align=center,font=\scriptsize,fill=blue!6,
                draw=blue!40},
  output/.style={draw,rectangle,rounded corners=3pt,minimum height=1.0cm,
                 minimum width=1.6cm,align=center,font=\scriptsize,fill=green!8,
                 draw=green!50},
  arr/.style={-Stealth,thick},
  lbl/.style={font=\tiny,align=center}
]
% Input signals
\node[input] (met) at (0,1.5) {\textbf{Metrics}\\Prometheus\\(15 s)};
\node[input] (log) at (0,0.0) {\textbf{Logs}\\Loki\\stream};
\node[input] (trc) at (0,-1.5){\textbf{Traces}\\Tempo\\OTLP};

% Pipeline stages
\node[stage] (col) at (2.5,0.0) {\textbf{Collector}\\normalise\\to MetricSignal};
\node[stage] (sp)  at (5.0,0.0) {\textbf{Stream}\\Processor\\60 s window};
\node[stage] (ml)  at (7.7,0.75){\textbf{Anomaly}\\Detector\\IsoForest};
\node[stage] (rca) at (7.7,0.0) {\textbf{RCA}\\Ranker\\Pearson};
\node[stage] (slo) at (7.7,-0.75){\textbf{SLO}\\Forecast\\OLS};
\node[stage] (llm) at (10.4,0.0){\textbf{LLM Engine}\\Qwen2.5-3B\\+ LoRA};
\node[stage] (dec) at (13.1,0.0){\textbf{Decision}\\Engine\\policy gating};
\node[output] (act) at (15.5,0.0){\textbf{Remediation}\\K8s Actions\\/ Escalate};

% ML sub-box
\begin{scope}[on background layer]
  \node[draw=orange!40,dashed,rounded corners=3pt,fill=orange!3,inner sep=4pt,
        fit=(ml)(rca)(slo),
        label={[font=\tiny,orange!70!black]above:\textbf{ML Engine}}] (mlbox) {};
\end{scope}

% Arrows: inputs to collector
\draw[arr,blue!50] (met.east) -- ++(0.3,0) |- (col.west);
\draw[arr,blue!50] (log.east) -- (col.west);
\draw[arr,blue!50] (trc.east) -- ++(0.3,0) |- (col.west);
...
% Latency labels
\node[lbl,gray] at (3.75,-0.55) {Kafka\\$<$1 s};
\node[lbl,gray] at (6.2,-0.55) {window\\60 s};
\node[lbl,gray] at (9.05,-0.55) {LLM\\3--5 s};
\node[lbl,gray] at (11.85,-0.55) {K8s API\\$<$2 s};
\end{tikzpicture}%
}
...
\centering
\begin{tikzpicture}[
  box/.style={draw,rectangle,rounded corners=4pt,minimum height=0.85cm,
              minimum width=2.2cm,align=center,font=\scriptsize},
  ml/.style  ={box,fill=orange!8, draw=orange!55},
  llm/.style ={box,fill=purple!8, draw=purple!55},
  dec/.style ={box,fill=blue!8,   draw=blue!50},
  fb/.style  ={box,fill=green!8,  draw=green!50},
  arr/.style ={-Stealth,semithick},
  lbl/.style ={font=\tiny,align=center}
]
% Boxes placed in a horizontal loop
\node[ml]  (det) at (0.0, 0)   {\textbf{ML Engine}\\Fault Detected};
\node[llm] (nar) at (3.5, 0)   {\textbf{LLM Engine}\\Incident Report\\(JSON)};
\node[dec] (dec) at (7.0, 0)   {\textbf{Decision Engine}\\Action Selection};
\node[dec] (rem) at (10.5,0)   {\textbf{Remediation}\\K8s API Action};
\node[fb]  (out) at (10.5,-1.8){\textbf{Feedback Svc}\\Outcome Capture\\MLflow};
\node[fb]  (lra) at (3.5, -1.8){\textbf{LoRA Retraining}\\PEFT adapter\\upload};

% Forward arrows
\draw[arr] (det.east) -- node[above,lbl]{incident\\record} (nar.west);
\draw[arr] (nar.east) -- node[above,lbl]{structured\\report} (dec.west);
\draw[arr] (dec.east) -- node[above,lbl]{decision} (rem.west);

% Feedback loop
\draw[arr] (rem.south) -- (out.north);
\draw[arr] (out.west)  -- node[below,lbl]{\textsc{resolved}\\label} (lra.east);
\draw[arr,dashed,orange!70!black] (lra.north)
    -- node[left,lbl]{adapter\\weights} (nar.south);

% Input arrow
\draw[arr,blue!50] (-2.0,0) -- node[above,lbl]{fault\\onset} (det.west);
\end{tikzpicture}
...
%% ── Panel borders ────────────────────────────────────────────────────
\draw[gray!35,rounded corners=4pt,fill=gray!4] (-0.1,-0.2) rectangle (3.9,4.4);
\draw[gray!35,rounded corners=4pt,fill=gray!4] (4.1,-0.2) rectangle (8.1,4.4);
\draw[gray!35,rounded corners=4pt,fill=gray!4] (8.3,-0.2) rectangle (12.3,4.4);

%% ── Panel 1: RQ1 Detection F1 ────────────────────────────────────────
\node[font=\scriptsize\bfseries] at (1.9,4.0) {RQ1: Detection F1};
% Axes
\draw[->] (0.4,0.4)--(3.6,0.4) node[right,font=\tiny]{Modality};
\draw[->] (0.4,0.4)--(0.4,3.5) node[above,font=\tiny]{F1};
% Schematic bars (metrics-only, M+L, all three)
\fill[blue!25] (0.7,0.4) rectangle (1.3,1.5);
\fill[blue!45] (1.6,0.4) rectangle (2.2,2.4);
\fill[blue!70] (2.5,0.4) rectangle (3.1,3.2);
\node[font=\tiny] at (1.0,0.15) {M};
\node[font=\tiny] at (1.9,0.15) {M+L};
\node[font=\tiny] at (2.8,0.15) {All};
\draw[gray!50,dashed] (0.4,3.0)--(3.5,3.0);
\node[font=\tiny,gray] at (0.2,3.0) {1.0};
\node[font=\tiny,gray] at (0.2,0.4) {0};
\node[font=\tiny,gray!70,align=center] at (1.9,3.7) {\textit{schematic;}\\
  \textit{results pending}};

%% ── Panel 2: RQ2 RCA Top-1 Accuracy ─────────────────────────────────
\node[font=\scriptsize\bfseries] at (6.1,4.0) {RQ2: RCA Accuracy};
\draw[->] (4.6,0.4)--(7.8,0.4) node[right,font=\tiny]{Method};
\draw[->] (4.6,0.4)--(4.6,3.5) node[above,font=\tiny]{Top-1 acc.};
\fill[blue!30]   (4.8,0.4) rectangle (5.4,1.3);
\fill[orange!50] (5.7,0.4) rectangle (6.3,2.0);
\fill[green!55]  (6.6,0.4) rectangle (7.2,3.1);
\node[font=\tiny] at (5.1,0.15) {Thr.};
\node[font=\tiny] at (6.0,0.15) {Trace};
\node[font=\tiny] at (6.9,0.15) {Pearson};
\draw[gray!50,dashed] (4.6,3.0)--(7.7,3.0);
\node[font=\tiny,gray] at (4.35,3.0) {1.0};
\node[font=\tiny,gray] at (4.35,0.4) {0};
\node[font=\tiny,gray!70,align=center] at (6.1,3.7) {\textit{schematic;}\\
  \textit{results pending}};

%% ── Panel 3: RQ3 Tracing Overhead ────────────────────────────────────
\node[font=\scriptsize\bfseries] at (10.3,4.0) {RQ3: Trace Overhead};
\draw[->] (8.6,0.4)--(12.0,0.4) node[right,font=\tiny]{Sample \%};
\draw[->] (8.6,0.4)--(8.6,3.5) node[above,font=\tiny]{p95 lat. (ms)};
\draw[red!60,thick] (8.9,0.7)--(9.6,1.1)--(10.3,1.8)--(11.0,2.4)--(11.7,3.1);
\foreach \x/\l in {8.9/0,9.6/10,10.3/50,11.0/75,11.7/100}{
  \node[font=\tiny] at (\x,0.15) {\l};}
\draw[gray!50,dashed] (8.6,2.0)--(11.9,2.0)
  node[right,font=\tiny,gray]{SLO};
\node[font=\tiny,gray] at (8.35,0.4) {0};
\node[font=\tiny,gray!70,align=center] at (10.3,3.7) {\textit{schematic;}\\
  \textit{results pending}};
\end{tikzpicture}
...
across trace sampling rates 0--100\%.}
\label{fig:eval}
\end{figure}

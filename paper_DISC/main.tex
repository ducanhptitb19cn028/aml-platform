...
\midrule
\texttt{aml.\{customers,transactions,cases\}.events} & AML services & Telemetry Collector\\
\texttt{aml.llm.analysis}                            & LLM Engine   & Decision Engine\\
\texttt{aiops.telemetry.\{metrics,logs,traces\}}     & AML services / Prometheus & LLM Engine\\
...
Table~\ref{tab:components} summarises all eight AIOps analysis services. Three
component implementations merit elaboration for their algorithmic novelty.
...
(${<}2$\,s) $\approx$ \textbf{83--85\,s} worst-case.
...
programme. Fig.~\ref{fig:eval} shows the preliminary experimental setup for the
three research questions. Preliminary system validation confirms: (i)~the telemetry
pipeline correctly ingests and normalises events from all eleven services, with
30-second heartbeat signals confirmed on \texttt{aiops.service.heartbeat} for every
AIOps service; (ii)~the ML Engine detects injected CPU stress anomalies within two
60-second analysis cycles (${\approx}120$\,s) by observing the \texttt{avg\_value}
and \texttt{rate\_of\_change} feature dimensions; (iii)~health scoring reflects
per-service anomaly and CPU states consistently with configured load profiles;
and (iv)~the LLM Engine produces valid JSON incident reports conforming to the
output schema for all test telemetry contexts, with Qwen2.5-3B running entirely
on-cluster at approximately 3--5\,s per inference on CPU. Zero external network
calls were observed throughout the detection-to-report pipeline---verified by
Kubernetes NetworkPolicy audit logs showing zero egress rejections during a 2-hour
fault-injection session---validating the data-residency compliance posture required
for AML environments.

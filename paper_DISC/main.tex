...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  svc/.style={draw,rectangle,rounded corners=3pt,minimum height=0.75cm,
              minimum width=2.2cm,align=center,font=\small,fill=gray!10,
              draw=gray!55,line width=0.7pt},
  pillar/.style={draw,rectangle,rounded corners=5pt,minimum height=4.5cm,
                 minimum width=3.0cm,align=center,font=\small,
                 line width=0.8pt,inner sep=6pt},
  intel/.style={draw,rectangle,rounded corners=5pt,minimum height=0.95cm,
                align=center,font=\small,line width=0.8pt,inner sep=6pt},
  arr/.style={-Stealth,thick,line width=1.2pt}
]
%% Application layer (y = 6.0)
\node[svc] (s1) at (1.3, 6.0) {KYC};
\node[svc] (s2) at (3.8, 6.0) {Tx-Mon};
\node[svc] (s3) at (6.3, 6.0) {Cases};
\node[svc] (s4) at (8.8, 6.0) {Stream};
\node[font=\small] at (10.2,6.0) {\ldots};
\draw[gray!55,dashed,rounded corners=4pt,line width=0.7pt]
    (0.1,5.55) rectangle (10.8,6.5);
\node[font=\small\bfseries,fill=white,inner sep=2pt] at (5.45,6.5)
    {Cloud-Native Application (Kubernetes)};
\node[font=\scriptsize,fill=white,inner sep=1pt] at (5.45,5.55)
    {OpenTelemetry Auto-Instrumentation};

%% Three pillars (centered at y = 2.6)
\node[pillar,fill=blue!6,draw=blue!50] (M) at (1.8,2.6) {
    \textbf{Metrics}\\[6pt]
    CPU / Memory\\
    Request rate\\
    Error rate\\[10pt]
...
\node[pillar,fill=green!6,draw=green!50] (L) at (5.4,2.6) {
    \textbf{Logs}\\[6pt]
    Events\\
    Stack traces\\
    Warnings\\[10pt]
...
\node[pillar,fill=orange!6,draw=orange!50] (T) at (9.0,2.6) {
    \textbf{Traces}\\[6pt]
    Span latency\\
    Call graph\\
    Error status\\[10pt]
...
%% Intelligence layer (y = -0.5)
\node[intel,draw=purple!60,fill=purple!5,minimum width=11.5cm] (AI) at (5.4,-0.5) {
    \textbf{Observability Intelligence Layer:}\quad
    Anomaly Detection $\;\mid\;$ RCA $\;\mid\;$ Forecasting
};

%% Arrows: services → pillars
\draw[arr,gray!60] (s1.south) -- ++(0,-0.35) -| (M.north);
\draw[arr,gray!60] (s2.south) -- ++(0,-0.35) -| (L.north);
\draw[arr,gray!60] (s3.south) -- ++(0,-0.35) -| (L.north);
\draw[arr,gray!60] (s4.south) -- ++(0,-0.35) -| (T.north);

%% Arrows: pillars → intelligence
...
\draw[arr,dashed,orange!70!black,line width=1.3pt]
    (lra.north) -- node[left,lbl]{adapter\\weights} (nar.south);
...
time-to-detect for latency-class faults. Causal cascade faults (customer-kyc scaled
to zero) produce correlated metric drops across all three AML services, which are
visible in both metrics and traces simultaneously---consistent with the prediction
that cross-modal correlation is required for accurate root cause attribution~(RQ2).
...
namespaces. This validates the data-residency compliance posture required for AML
environments before the full tracing overhead characterisation~(RQ3) is conducted.

\subsection{Threats to Validity}

Three primary threats to validity apply to the current evaluation. \textit{Internal
validity}: the single-node Docker Desktop deployment constrains inter-pod network
latencies to near-zero, meaning measured detection latencies represent a best-case
bound; production multi-node deployments introduce additional Kafka propagation and
Prometheus scrape scheduling jitter that will lengthen end-to-end detection cycles.
\textit{Construct validity}: the synthetic k6 workload sustains uniform 50-user load
without the bursty traffic patterns characteristic of real AML deployments during
high-risk transaction periods; more realistic load profiles may produce different
baseline feature distributions, affecting Isolation Forest contamination calibration
and potentially requiring per-time-of-day model variants. \textit{External validity}:
results are obtained from a single Kubernetes cluster configuration running three AML
microservices; generalisation to multi-tenant, multi-region deployments or to AML
platforms with significantly different service dependency graphs and larger
$N$---where the $O(|\mathcal{N}(s_{\text{anom}})| \cdot K)$ RCA search space grows
substantially---requires further evaluation across platforms with realistic
transaction volumes.
...
%% Panel geometry: each panel 6.2cm wide x 6.2cm tall, 0.5cm gap
%% P1: x -0.3..5.9   P2: x 6.4..12.6   P3: x 13.1..19.3
\draw[gray!35,rounded corners=5pt,fill=gray!4,line width=0.6pt]
    (-0.3,-0.4) rectangle (5.9,6.2);
\draw[gray!35,rounded corners=5pt,fill=gray!4,line width=0.6pt]
    (6.4,-0.4) rectangle (12.6,6.2);
\draw[gray!35,rounded corners=5pt,fill=gray!4,line width=0.6pt]
    (13.1,-0.4) rectangle (19.3,6.2);

%% ── Panel 1: RQ1 Detection F1 ────────────────────────────────────────
\node[font=\small\bfseries] at (2.8,5.7) {RQ1: Detection F1};
\draw[->,line width=0.8pt] (0.3,0.6)--(5.5,0.6) node[right,font=\scriptsize]{Modality};
\draw[->,line width=0.8pt] (0.3,0.6)--(0.3,5.4) node[above,font=\scriptsize]{F1 score};
%% Bars with outlines
\fill[blue!22] (0.8,0.6) rectangle (2.0,2.1); \draw[blue!50,line width=0.5pt] (0.8,0.6) rectangle (2.0,2.1);
\fill[blue!45] (2.3,0.6) rectangle (3.5,3.4); \draw[blue!60,line width=0.5pt] (2.3,0.6) rectangle (3.5,3.4);
\fill[blue!72] (3.8,0.6) rectangle (5.0,5.0); \draw[blue!75,line width=0.5pt] (3.8,0.6) rectangle (5.0,5.0);
\node[font=\scriptsize] at (1.4,0.28) {M only};
\node[font=\scriptsize] at (2.9,0.28) {M+L};
\node[font=\scriptsize] at (4.4,0.28) {All 3};
\draw[gray!45,dashed,line width=0.7pt] (0.3,5.0)--(5.4,5.0);
\node[font=\scriptsize,gray!60] at (-0.1,5.0) {1.0};
\node[font=\scriptsize,gray!60] at (-0.1,0.6) {0};
\node[font=\scriptsize,gray!65,align=center] at (2.8,5.4) {\textit{schematic; results pending}};

%% ── Panel 2: RQ2 RCA Top-1 Accuracy ─────────────────────────────────
\node[font=\small\bfseries] at (9.5,5.7) {RQ2: RCA Accuracy};
\draw[->,line width=0.8pt] (7.0,0.6)--(12.2,0.6) node[right,font=\scriptsize]{Method};
\draw[->,line width=0.8pt] (7.0,0.6)--(7.0,5.4) node[above,font=\scriptsize]{Top-1 acc.};
\fill[blue!28]   (7.4,0.6) rectangle (8.6,2.0); \draw[blue!50,line width=0.5pt] (7.4,0.6) rectangle (8.6,2.0);
\fill[orange!52] (8.9,0.6) rectangle (10.1,3.2); \draw[orange!60,line width=0.5pt] (8.9,0.6) rectangle (10.1,3.2);
\fill[green!58]  (10.4,0.6) rectangle (11.6,4.9); \draw[green!65,line width=0.5pt] (10.4,0.6) rectangle (11.6,4.9);
\node[font=\scriptsize] at (8.0,0.28) {Threshold};
\node[font=\scriptsize] at (9.5,0.28) {Trace};
\node[font=\scriptsize] at (11.0,0.28) {Pearson};
\draw[gray!45,dashed,line width=0.7pt] (7.0,5.0)--(12.1,5.0);
\node[font=\scriptsize,gray!60] at (6.6,5.0) {1.0};
\node[font=\scriptsize,gray!60] at (6.6,0.6) {0};
\node[font=\scriptsize,gray!65,align=center] at (9.5,5.4) {\textit{schematic; results pending}};

%% ── Panel 3: RQ3 Tracing Overhead ────────────────────────────────────
\node[font=\small\bfseries] at (16.2,5.7) {RQ3: Trace Overhead};
\draw[->,line width=0.8pt] (13.6,0.6)--(18.9,0.6) node[right,font=\scriptsize]{Sample rate (\%)};
\draw[->,line width=0.8pt] (13.6,0.6)--(13.6,5.4) node[above,font=\scriptsize]{p95 lat. (ms)};
%% Line chart with filled markers
\draw[red!60,thick,line width=1.5pt]
    (14.0,0.9)--(14.9,1.5)--(15.8,2.3)--(16.7,3.2)--(17.6,4.5);
\fill[red!60] (14.0,0.9) circle (3.5pt);
\fill[red!60] (14.9,1.5) circle (3.5pt);
\fill[red!60] (15.8,2.3) circle (3.5pt);
\fill[red!60] (16.7,3.2) circle (3.5pt);
\fill[red!60] (17.6,4.5) circle (3.5pt);
\node[font=\scriptsize] at (14.0,0.28) {0};
\node[font=\scriptsize] at (14.9,0.28) {10};
\node[font=\scriptsize] at (15.8,0.28) {50};
\node[font=\scriptsize] at (16.7,0.28) {75};
\node[font=\scriptsize] at (17.6,0.28) {100};
%% SLO reference line
\draw[gray!50,dashed,line width=0.9pt] (13.6,3.0)--(18.8,3.0)
    node[right,font=\scriptsize,gray!60]{SLO};
\node[font=\scriptsize,gray!60] at (13.2,3.0) {500};
\node[font=\scriptsize,gray!60] at (13.2,0.6) {0};
\node[font=\scriptsize,gray!65,align=center] at (16.2,5.4) {\textit{schematic; results pending}};
\end{tikzpicture}%
}
